...
public class CommentService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (dailyInstalledApps.size() == 0 || clickedApps.size() == 0) {
            return START_FLAG_RETRY;
        }
        //TODO: Decide if we want to let users comment bad apps or only good apps (which are still currently available on their phone)
        List<ApplicationInfo> currentlyExistingApps = PackagesUtils.getInstance().getInstalledPackages(getPackageManager());
        //List<InstalledApp> appsInstalledTodayFromAppHunt =
        for (int i = 0; i < dailyInstalledApps.size(); i++) {
            InstalledApp app = dailyInstalledApps.get(i);
